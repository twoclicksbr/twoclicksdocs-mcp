...
export function registerBlocksTools(server: McpServer) {
  server.tool(
    ...
    async ({ project, document_id, parent_id, search, expand, per_page }) => {
      try {
  ...
  server.tool(
    ...
    async ({ project, document_id, ...body }) => {
      try {
        const client = apiClient(project);
  ...
  server.tool(
    ...
    async ({ project, document_id, block_id, ...body }) => {
      try {
        const client = apiClient(project);
  ...
  server.tool(
    ...
    async ({ project, document_id, block_id }) => {
      try {
        const client = apiClient(project);
